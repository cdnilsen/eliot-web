...
app.post('/mark_cards_under_review', express.json(), wrapAsync(async (req, res) => {
    ...
    try {
        ...
        await buryRelatedCards(validCardIds, transactionClient);
...
async function buryRelatedCards(cardIds: number[], transactionClient: any): Promise<void> {
    console.log(`🔄 Burying related cards (peers/prereqs/dependents) for ${cardIds.length} cards under review`);
    
    try {
        // Get all relationships for the cards under review
        const relationshipsQuery = await transactionClient.query(
            `SELECT card_id, peers, prereqs, dependents 
             FROM cards 
             WHERE card_id = ANY($1::int[])`,
            [cardIds]
        );
        
        // Collect all unique related card IDs that need to be buried
        const relatedIdsToUpdate = new Set<number>();
        
        relationshipsQuery.rows.forEach((row: { 
            card_id: number; 
            peers: number[] | null;
            prereqs: number[] | null;
            ...
            if (row.peers && Array.isArray(row.peers)) {
                row.peers.forEach((peerId: number) => {
                    if (!cardIds.includes(peerId)) {
                        relatedIdsToUpdate.add(peerId);
                    }
                });
            }
            
            // Bury prereqs (cards this card depends on)
            if (row.prereqs && Array.isArray(row.prereqs)) {
                row.prereqs.forEach((prereqId: number) => {
                    if (!cardIds.includes(prereqId)) {
                        relatedIdsToUpdate.add(prereqId);
                    }
                });
            }
            
            // Bury dependents (cards that depend on this card)
            if (row.dependents && Array.isArray(row.dependents)) {
                row.dependents.forEach((depId: number) => {
                    if (!cardIds.includes(depId)) {
                        relatedIdsToUpdate.add(depId);
                    }
                });
            }
        });
        
        const uniqueRelatedIds = Array.from(relatedIdsToUpdate);
        
        if (uniqueRelatedIds.length > 0) {
            console.log(`📦 Burying ${uniqueRelatedIds.length} related cards:`, uniqueRelatedIds);
            ...
            console.log(`✅ Successfully buried ${uniqueRelatedIds.length} related cards`);
        } else {
            console.log(`ℹ️ No related cards found to bury`);
        }
        
    } catch (error) {
        console.error('❌ Error burying related cards:', error);
        throw error;
    }
}
...
// First, you'll need to add these columns to your cards table:
/*
ALTER TABLE cards ADD COLUMN IF NOT EXISTS dependents integer[] DEFAULT '{}';
ALTER TABLE cards ADD COLUMN IF NOT EXISTS prereqs integer[] DEFAULT '{}';
*/

// Add this endpoint to your Express app
app.post('/create_card_relationship', express.json(), wrapAsync(async (req, res) => {
    const { card_a_id, card_b_id, relationship, between_notes }: CreateCardRelationshipRequest = req.body;
    
    console.log(`🔗 Creating ${relationship} relationship between cards ${card_a_id} and ${card_b_id} (between_notes: ${between_notes})`);
    
    // Validation
    if (!card_a_id || !card_b_id || !relationship) {
        return res.json({
            status: 'error',
            error: 'card_a_id, card_b_id, and relationship are all required'
        } as CreateCardRelationshipResponse);
    }
    
    if (typeof card_a_id !== 'number' || typeof card_b_id !== 'number') {
        return res.json({
            status: 'error',
            error: 'card_a_id and card_b_id must be numbers'
        } as CreateCardRelationshipResponse);
    }
    
    if (!['peer', 'dependent', 'prereq'].includes(relationship)) {
        return res.json({
            status: 'error',
            error: 'relationship must be one of: peer, dependent, prereq'
        } as CreateCardRelationshipResponse);
    }
    ...
    if (card_a_id === card_b_id) {
        return res.json({
            status: 'error',
            error: 'A card cannot have a relationship with itself'
        } as CreateCardRelationshipResponse);
    }
    ...
    try {
        await transactionClient.query('BEGIN');
        console.log('🔗 Transaction started for relationship creation');
        
        // Verify both cards exist and get their note_ids
        const cardsQuery = await transactionClient.query(
            `SELECT card_id, note_id, peers, dependents, prereqs, deck 
             FROM cards 
             WHERE card_id IN ($1, $2)`,
            [card_a_id, card_b_id]
        );
        
        if (cardsQuery.rows.length !== 2) {
            await transactionClient.query('ROLLBACK');
            const foundIds = cardsQuery.rows.map(row => row.card_id);
            const missingIds = [card_a_id, card_b_id].filter(id => !foundIds.includes(id));
            return res.json({
                status: 'error',
        ...
        const cardA = cardsQuery.rows.find(row => row.card_id === card_a_id);
        const cardB = cardsQuery.rows.find(row => row.card_id === card_b_id);
        
        if (!cardA || !cardB) {
            await transactionClient.query('ROLLBACK');
            return res.json({
                status: 'error',
                error: 'Error retrieving card data'
            } as CreateCardRelationshipResponse);
        }
        
        console.log(`🔗 Found cards: ${cardA.card_id} (note: ${cardA.note_id}) and ${cardB.card_id} (note: ${cardB.note_id})`);
        
        let cardsToUpdate: { cardId: number; peers: number[] }[] = [];
    ...
    // Apply all accumulated updates to the database
        ...
        if (between_notes && relationship === 'peer') {
            console.log(`🔗 Successfully created peer relationships between notes`);
            console.log(`   Total relationships created: ${totalChanges.length}`);
            console.log(`   Unique cards updated: ${new Set(cardsToUpdate.map(u => u.cardId)).size}`);
        } else {
            console.log(`🔗 Successfully created ${relationship} relationship`);
            console.log(`   Changes: ${totalChanges.join(', ')}`);
        }
        ...
        res.json({
            ...
            changes_made: {
                card_a_changes: between_notes ? [`Created ${totalChanges.length} note-level relationships`] : totalChanges.filter(c => c.includes(`${card_a_id}`)),
                card_b_changes: between_notes ? [`Participated in ${totalChanges.length} note-level relationships`] : totalChanges.filter(c => c.includes(`${card_b_id}`))
            }
        } as CreateCardRelationshipResponse);
        ...
        res.json({
            status: 'error',
            error: 'Failed to create card relationship',
            details: error instanceof Error ? error.message : 'Unknown error occurred'
        } as CreateCardRelationshipResponse);
    } finally {
        transactionClient.release();
...
// Bonus: Add an endpoint to remove relationships
app.post('/remove_card_relationship', express.json(), wrapAsync(async (req, res) => {
    const { card_a_id, card_b_id, relationship }: CreateCardRelationshipRequest = req.body;
    
    console.log(`🔗❌ Removing ${relationship} relationship between cards ${card_a_id} and ${card_b_id}`);
    ...
    try {
        ...
        // Get current card data
        const cardsQuery = await transactionClient.query(
            `SELECT card_id, peers, dependents, prereqs 
             FROM cards 
             WHERE card_id IN ($1, $2)`,
            [card_a_id, card_b_id]
        );
        ...
        switch (relationship) {
            case 'peer':
                // Remove each other from peers arrays
                const newCardAPeers = (cardA.peers || []).filter((id: number) => id !== card_b_id);
                const newCardBPeers = (cardB.peers || []).filter((id: number) => id !== card_a_id);
                
                if (newCardAPeers.length !== (cardA.peers || []).length) {
                    cardAChanges.push(`Removed card ${card_b_id} from peers`);
                }
                if (newCardBPeers.length !== (cardB.peers || []).length) {
                    cardBChanges.push(`Removed card ${card_a_id} from peers`);
                }
                
                await transactionClient.query('UPDATE cards SET peers = $1 WHERE card_id = $2', [newCardAPeers, card_a_id]);
                await transactionClient.query('UPDATE cards SET peers = $1 WHERE card_id = $2', [newCardBPeers, card_b_id]);
                break;
    ...
    // Update the arrays and log changes...
        ...
        console.log(`🔗❌ Successfully removed ${relationship} relationship`);
        console.log(`   Card ${card_a_id} changes: ${cardAChanges.join(', ') || 'None'}`);
        console.log(`   Card ${card_b_id} changes: ${cardBChanges.join(', ') || 'None'}`);
        ...
        res.json({
            ...
            relationship: relationship,
            changes_made: {
                card_a_changes: cardAChanges,
                card_b_changes: cardBChanges
            }
        } as CreateCardRelationshipResponse);
        ...
        res.json({
            status: 'error',
            error: 'Failed to remove card relationship',
            details: error instanceof Error ? error.message : 'Unknown error occurred'
        } as CreateCardRelationshipResponse);
    } finally {
        transactionClient.release();
...
// Add this interface near your other type definitions in index.ts
interface PreviewTodaysCardsResponse {
    status: 'success' | 'error';
    deck?: string;
    cards?: any[];
    total_count?: number;
...
// Add this endpoint to your Express app (before the app.listen call)
app.get('/preview_todays_cards/:deckName', wrapAsync(async (req, res) => {
    const { deckName } = req.params;
    ...
    if (!deckName || typeof deckName !== 'string') {
        return res.status(400).json({
            status: 'error',
            error: 'Deck name is required'
        } as PreviewTodaysCardsResponse);
    }
    
    console.log(`👀 Previewing today's cards for deck: "${deckName}"`);
    
    // Get today's date range (start and end of today in local timezone)
    const now = new Date();
    const startOfToday = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    const endOfToday = new Date(startOfToday.getTime() + 24 * 60 * 60 * 1000); // Add 24 hours
    
    console.log(`📅 Looking for cards created between: ${startOfToday.toISOString()} and ${endOfToday.toISOString()}`);
    ...
    try {
        const query = await client.query(
            ...
            AND created >= $2 
            AND created < $3
            ORDER BY created ASC, card_id ASC`,
            [deckName, startOfToday, endOfToday]
        );
        
        console.log(`📊 Found ${query.rows.length} cards created today in deck "${deckName}"`);
        
        // Log some examples if there are cards
        if (query.rows.length > 0) {
            console.log(`📝 First few cards:`, query.rows.slice(0, 3).map(card => ({
                card_id: card.card_id,
                created: card.created,
                field_values: card.field_values?.slice(0, 2) // Show first 2 fields only
            })));
        }
        ...
        console.error('Error previewing today\'s cards:', err);
        res.status(500).json({
            status: 'error',
            error: 'Error previewing today\'s cards',
            details: err instanceof Error ? err.message : 'Unknown error',
            deck: deckName
        } as PreviewTodaysCardsResponse);
    }
...
// Optional: Add an endpoint to get a summary of today's card creation across all decks
app.get('/todays_cards_summary', wrapAsync(async (req, res) => {
    console.log(`📊 Getting summary of today's card creation across all decks`);
    ...
    try {
        const summaryQuery = await client.query(`
            SELECT 
                deck,
                COUNT(*) as cards_created_today,
                MIN(created) as first_card_time,
                MAX(created) as last_card_time
            FROM cards 
            WHERE created >= $1 
            AND created < $2
            GROUP BY deck
            ORDER BY cards_created_today DESC, deck ASC
        `, [startOfToday, endOfToday]);
        
        const totalQuery = await client.query(`
            SELECT COUNT(*) as total_cards_today
            FROM cards 
            WHERE created >= $1 
            AND created < $2
        `, [startOfToday, endOfToday]);
        
        const totalCardsToday = parseInt(totalQuery.rows[0].total_cards_today) || 0;
        
        console.log(`📊 Total cards created today: ${totalCardsToday} across ${summaryQuery.rows.length} decks`);
        ...
        res.json({
            ...
            total_cards_today: totalCardsToday,
            decks_with_new_cards: summaryQuery.rows.length,
            deck_breakdown: summaryQuery.rows.map(row => ({
                deck: row.deck,
                cards_created: parseInt(row.cards_created_today),
                first_card_time: row.first_card_time,
                last_card_time: row.last_card_time
            }))
        });
        
    } catch (err) {
        console.error('Error getting today\'s cards summary:', err);
        res.status(500).json({
            status: 'error',
            error: 'Error getting today\'s cards summary',
            details: err instanceof Error ? err.message : 'Unknown error'
        });
    }
}));
