...
import {
  transliterateGeez,
  GeezDiacriticify,
} from "./synapdeck_files/transcribe_geez.js";
...
async function loadDeckFronts(deck: string): Promise<void> {
  ...
  try {
    const response = await fetch(
      `/deck_fronts?deck=${encodeURIComponent(deck)}`,
    );
...
function revalidateSpreadsheetConflicts(): void {
  ...
  Array.from(tbody.rows).forEach((row) =>
    checkRowConflicts(row as HTMLTableRowElement),
  );
}

function checkRowConflicts(row: HTMLTableRowElement): void {
  const cardFormatEl = document.getElementById(
    "card_format_dropdown",
  ...
  Array.from(tbody.rows).forEach((row, rowIdx) => {
    ...
    dataCells.forEach((cell, colIdx) => {
      if (colIdx >= minCol && colIdx <= maxCol)
        cells.push(cell as HTMLTableDataCellElement);
    });
  });
  return cells;
}
...
function _clearHeaderRowIndicators(): void {
  if (activeHeaderTh) {
    activeHeaderTh.classList.remove("col-header-active");
    activeHeaderTh = null;
  }
  if (activeRowNumTd) {
    activeRowNumTd.classList.remove("row-num-active");
    activeRowNumTd = null;
  }
}
...
function _clearRangeHighlight(): void {
  ...
  if (!tbody) return;
  tbody
    .querySelectorAll("td.cell-in-range, td.cell-in-range-empty")
    .forEach((td) => {
...
function _applyRangeHighlight(): void {
  ...
  if (
    !selectionAnchorTd ||
    !selectionActiveTd ||
    selectionAnchorTd === selectionActiveTd
  )
    return;
...
function _extendSelection(dir: "up" | "down" | "left" | "right"): void {
  ...
  const dataCells = Array.from(rows[row].cells)
    .slice(1)
    .filter((c) => !c.classList.contains("initial-interval-cell"));
...
function _captureGridSnapshot(): GridSnapshot {
  ...
  const cardType = cardFormatEl?.value ?? "two-way";
  const numCols = (SPREADSHEET_COLS[cardType] ?? SPREADSHEET_COLS["two-way"])
    .length;
  return Array.from(tbody.rows).map((row) => {
    const values: string[] = [];
    for (let i = 1; i <= numCols; i++) {
      const ta = row.cells[i]?.querySelector(
        "textarea",
...
function _restoreSnapshot(idx: number): void {
  ...
  Array.from(tbody.rows).forEach((row, rowIdx) => {
    ...
    for (let i = 0; i < numCols; i++) {
      const ta = row.cells[i + 1]?.querySelector(
        "textarea",
      ) as HTMLTextAreaElement | null;
      if (ta) {
...
function buildSpreadsheet(cardType: string): void {
  ...
  const colWidths =
    SPREADSHEET_COL_WIDTHS[cardType] ?? SPREADSHEET_COL_WIDTHS["two-way"];
  ...
  cols.forEach((col, colIdx) => {
    ...
    th.addEventListener("mousedown", (e) => {
      ...
      const rows = Array.from(tbody.rows) as HTMLTableRowElement[];
      const firstCell = rows[0].cells[colIdx + 1] as
        | HTMLTableDataCellElement
        | undefined;
      const lastCell = rows[rows.length - 1].cells[colIdx + 1] as
        | HTMLTableDataCellElement
        | undefined;
...
function addSpreadsheetRow(): void {
  ...
  tdNum.addEventListener("mousedown", (e) => {
    ...
    const dataCells = Array.from(tr.cells)
      .slice(1)
      .filter(
        (c) => !c.classList.contains("initial-interval-cell"),
  ...
  cols.forEach((_col, colIdx) => {
    ...
    textarea.addEventListener("blur", () => {
      ...
      setTimeout(() => {
        ...
        if (
          td.classList.contains("cell-editing") &&
...
function addRelEntry(primaryField: string, deck: string = currentDeck): void {
  ...
  const rels = rowRelationships.get(currentRelRow) ?? [];
  if (rels.some((r) => r.type === type && r.primaryField === primaryField))
    return;
...
function syncConflictColumnHeights(): void {
  ...
  const spacer = document.getElementById(
    "conflictHeaderSpacer",
  ) as HTMLElement | null;
  const relSpacer = document.getElementById(
    "relHeaderSpacer",
  ) as HTMLElement | null;
  const container = document.getElementById("cardSpreadsheet");
  if (spacer && container) {
...
function getNotesFromSpreadsheet(): NoteToProcess[] {
  ...
  Array.from(tbody.rows).forEach((row, rowIdx) => {
    ...
    const intervalCell = allCells.find((td) =>
      td.classList.contains("initial-interval-cell"),
    );
    const dataCells = allCells.filter(
      (td) => !td.classList.contains("initial-interval-cell"),
    );
    const fieldValues = dataCells.map((td) => {
      const ta = td.querySelector("textarea");
      return ta ? ta.value.trim() : "";
    });
    const intervalInputEl = intervalCell?.querySelector(
      "input",
    ) as HTMLInputElement | null;
    const noteInitialIntervalDays = Math.max(
      1,
      parseInt(intervalInputEl?.value ?? "1", 10) || 1,
    );
    ...
    const relationships: RelationshipsWithDecks = {
      peers: [],
      prereqs: [],
      dependents: [],
    };
    ...
    if (relRow) {
      for (const rel of rowRelationships.get(relRow) ?? []) {
        ...
        else if (rel.type === "dependent")
          relationships.prereqs.push(target); // "dependent of X" → X is our prereq
...
function getSpreadsheetCellTextarea(
  row: HTMLTableRowElement,
  dataColIdx: number,
): HTMLTextAreaElement | null {
  const cell = row.cells[dataColIdx + 1]; // +1 to skip row-number cell
  return cell
    ? (cell.querySelector("textarea") as HTMLTextAreaElement | null)
    : null;
}

function moveSpreadsheetFocusVertical(
  ...
  if (!targetRow) return;
  const targetCell = targetRow.cells[dataColIdx + 1] as
    | HTMLTableDataCellElement
    | undefined;
  if (targetCell) selectCell(targetCell);
}

function moveSpreadsheetFocusHorizontal(
  currentRow: HTMLTableRowElement,
  dataColIdx: number,
  dir: -1 | 1,
): void {
  ...
  const targetCell = targetRow.cells[targetColIdx + 1] as
    | HTMLTableDataCellElement
    | undefined;
  if (targetCell) selectCell(targetCell);
}
...
function transliterateSelectedCell(): void {
  ...
  const saved =
    savedTextareaSelection?.ta === ta ? savedTextareaSelection : null;
  const selStart = liveStart !== liveEnd ? liveStart : (saved?.start ?? 0);
  const selEnd =
    liveStart !== liveEnd ? liveEnd : (saved?.end ?? ta.value.length);
...
async function sendNoteToBackend(
  ...
  const initialDueOffsets: number[] = Array.from(
...
async function findCardByPrimaryField(
  deck: string,
  primaryValue: string,
): Promise<{ card_id: number } | null> {
  try {
...
async function processAllRelationships(
  cardsWithRelationships: Array<{
    cardIds: number[];
    deck: string;
    primaryField: string;
    relationships: RelationshipsWithDecks;
  ...
  for (const cardSet of cardsWithRelationships) {
    ...
    for (const cardId of cardIds) {
      for (const peer of relationships.peers) {
        try {
          const peerResult = await findCardByPrimaryField(
            peer.deck,
            peer.primaryField,
          );
          if (peerResult && peerResult.card_id) {
            await _createCardRelationship(
              cardId,
              peerResult.card_id,
              "peer",
              true,
            ...
            console.warn(
              `Peer card not found: "${peer.primaryField}" in deck "${peer.deck}"`,
            );
          }
        } catch (error) {
          console.error(
            `Error creating peer relationship for "${peer.primaryField}":`,
            error,
          );
        }
        await delay(50);
      ...
      for (const prereq of relationships.prereqs) {
        try {
          const prereqResult = await findCardByPrimaryField(
            prereq.deck,
            prereq.primaryField,
          );
          if (prereqResult && prereqResult.card_id) {
            await _createCardRelationship(
              cardId,
              prereqResult.card_id,
              "dependent",
              true,
            );
          } else {
            console.warn(
              `Prereq card not found: "${prereq.primaryField}" in deck "${prereq.deck}"`,
            );
          }
        } catch (error) {
          console.error(
            `Error creating prereq relationship for "${prereq.primaryField}":`,
            error,
          );
        }
        await delay(50);
      ...
      for (const dep of relationships.dependents) {
        try {
          const depResult = await findCardByPrimaryField(
            dep.deck,
            dep.primaryField,
          );
          if (depResult && depResult.card_id) {
            ...
            console.warn(
              `Dependent card not found: "${dep.primaryField}" in deck "${dep.deck}"`,
            );
          }
        } catch (error) {
          console.error(
            `Error creating dependent relationship for "${dep.primaryField}":`,
            error,
          );
        }
        await delay(50);
...
function initializeSpreadsheet(): void {
  const cardFormatDropdown = document.getElementById(
    "card_format_dropdown",
  ) as HTMLSelectElement;
...
export function initializeUploadTab(
  createCardRelationshipFn: CreateCardRelFn,
): void {
  _createCardRelationship = createCardRelationshipFn;

  const timestampCreated = new Date(Date.now()).toISOString();

  let uploadDeckDropdown = document.getElementById(
    "upload_dropdownMenu",
  ) as HTMLSelectElement;
  const cardFormatDropdown = document.getElementById(
    "card_format_dropdown",
  ) as HTMLSelectElement;
  let uploadSubmitButton = document.getElementById(
    "upload_submitBtn",
  ) as HTMLButtonElement;

  if (uploadDeckDropdown) {
    // Clone to remove any stale listeners
    const newUploadDropdown = uploadDeckDropdown.cloneNode(
      true,
    ) as HTMLSelectElement;
    if (uploadDeckDropdown.parentNode) {
      uploadDeckDropdown.parentNode.replaceChild(
        newUploadDropdown,
        uploadDeckDropdown,
      );
      // Re-query after DOM replacement
      uploadSubmitButton = document.getElementById(
        "upload_submitBtn",
      ) as HTMLButtonElement;
    }
  ...
  if (uploadSubmitButton) {
    uploadSubmitButton.addEventListener("click", async () => {
        ...
        try {
          ...
          if (result.status === "success") {
            ...
            const hasRelationships =
              note.relationships.peers.length > 0 ||
      ...
      const markAllPeersCheckbox = document.getElementById(
        "markAllPeers",
      ) as HTMLInputElement | null;
      if (
        markAllPeersCheckbox?.checked &&
        _createCardRelationship &&
        createdRepresentativeIds.length > 1
      ) {
        for (let i = 0; i < createdRepresentativeIds.length; i++) {
          for (let j = i + 1; j < createdRepresentativeIds.length; j++) {
            await _createCardRelationship(
              createdRepresentativeIds[i],
              createdRepresentativeIds[j],
              "peer",
              true,
            );
          }
