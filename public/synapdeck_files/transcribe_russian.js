...
// Build lookup maps
const russianInputToInternal = {};
const russianInternalToOutput = {};
...
function maximumMunchTokenizeRussian(str) {
    let strCursor = 0;
    const out = [];
...
export function transliterateRussian(str) {
    str = preprocessLatin(str);
    const tokens = maximumMunchTokenizeRussian(str);
    return tokens.map(token => renderRussian(token)).join("");
}
export let russianSpecialChars = ["ž", "š", "č", "ʹ", "ʺ", "ë", "è", "ě"];
export function RussianDiacriticify(str, isASCII) {
    if (isASCII) {
...
export function handleRussianPhonemicChanges(str) {
...
export function preprocessRussianText(str, applyPhonemic = false) {
    str = preprocessLatin(str);
    if (applyPhonemic) {
...
//# sourceMappingURL=transcribe_russian.js.map